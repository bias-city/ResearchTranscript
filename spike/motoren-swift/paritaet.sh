...
echo "################ (b) Ausschnitt gegen ffmpeg -ss -t"
clip() { # datei start dauer
  n=$(basename $1 | tr . _)_${2}_${3}
  $FF -y -v error -ss $2 -t $3 -i $1 -vn -ar 16000 -ac 1 -c:a pcm_s16le $O/refclip_$n.wav
  echo "== clip $n"
  $P wav16k $1 $O/rtclip_$n.wav $2 $3
  $V $O/rtclip_$n.wav $O/refclip_$n.wav
  # Lage im Volldekodat (ffmpeg-Referenz der ganzen Datei): Versatz gegen ref ab Sekunde start
  full=$O/ref_$(basename $1 | tr . _).wav
  [[ -f $full ]] && python3 - $O/rtclip_$n.wav $full $2 <<'PY'
import sys, numpy as np, wave
def lese(p):
    w = wave.open(p); return np.frombuffer(w.readframes(w.getnframes()), dtype=np.int16).astype(np.float64)
c, f, start = lese(sys.argv[1]), lese(sys.argv[2]), float(sys.argv[3])
i0 = int(round(start * 16000)); m = min(len(c), 16000)
best, bestv = 0, -1
...
print(f"   Lage im ffmpeg-Volldekodat: {start + best/16000:.4f} s (Soll {start:.3f}, Abweichung {best/16000*1000:+.1f} ms, r={bestv:.4f})")
PY
}
clip $F/interview.mp3 30 8            # Hörprobe aus dem Bibliotheks-mp3
clip $F/interview.mp3 0 5
clip $F/interview.mp3 120 10          # über das Ende hinaus
clip $F/video_hevc.mov 30 1
clip $O/interview_stereo.m4a 10 1
clip $F/interview.ogg 30 1            # Seeking im Reader unbrauchbar → Neustart von vorn
clip $O/ref_interview_mp3.wav 12.345 3.21   # jobs._clip: Ausschnitt aus dem 16-kHz-WAV
...
echo "== drei Aufrufe warm im selben Prozess (parallel-diarize k=1 dreimal ist nicht warm; probe hat keine Wiederholung) — siehe (4)"

echo "################ (e) Abbruch"
rm -f $O/abbruch_*.wav $O/abbruch_*.mp3
...
echo "== diarize feld5, Abbruch nach 0.05 s (während des Modell-Ladens)"; RT_ABBRUCH_NACH_S=0.05 $P diarize $O/ref_feld5.wav $M 0 $O/abbruch2.rttm
...
RT_NUR_WAV=1 $P parallel $O/ref_demo.wav $M $O/long.mp3 $O/par4_1.wav $O/long.mp3 $O/par4_2.wav $O/feld5.mp3 $O/par4_3.wav $F/video_hevc.mov $O/par4_4.wav > /dev/null
