...
**App Store:** the same code is distributed through the Mac App Store
under an additional permission (AGPL §7) in `LICENSE-EXCEPTION`; the
source stays AGPL. Every bundled component is listed with its licence
text in `THIRD_PARTY_LICENSES.md` (generated by
`scripts/gen-licenses.py`, shipped inside the app, button "All
licences" in the settings).
